...
#------------------------------------------------------------
# UPDATED iter4:
#   - Empirical p uses (k+1)/(B+1) correction; always > 0
#   - Asymptotic p computed from z-score assuming normal null
#   - z-score provided for reporting (e.g., "z = 5.2, p < 0.0001")
#------------------------------------------------------------
wr_perm_p <- function(dat, B = 5000, active_val = 1, seed = 1) {
  ...
  # --- Helper: compute empirical and asymptotic p-values ---
  compute_pvals <- function(t_obs, perm_dist, B) {
    # Count how many permutation values are as or more extreme than observed
    k <- sum(abs(perm_dist) >= abs(t_obs), na.rm = TRUE)

    # Empirical p with (k+1)/(B+1) correction; guarantees p > 0
    p_emp <- (k + 1) / (B + 1)

    # Asymptotic p from z-score (assumes approx normal null distribution)
    # z measures how many SDs the observed statistic is from the null mean
    null_mean <- mean(perm_dist, na.rm = TRUE)
    null_sd   <- sd(perm_dist, na.rm = TRUE)
    z <- (t_obs - null_mean) / null_sd
    p_asymp <- 2 * pnorm(-abs(z))
  ...
  pv_wr <- compute_pvals(t_obs_wr, perm_results$log_wr, B)
  pv_wo <- compute_pvals(t_obs_wo, perm_results$log_wo, B)
  pv_nb <- compute_pvals(t_obs_nb, perm_results$nb, B)

  list(
    # Observed values (original scale)
    wr_obs = wr_obs,
    wo_obs = wo_obs,
    nb_obs = nb_obs,
    # Test statistics (analysis scale: log for ratios, identity for NB)
    t_obs_wr = t_obs_wr,
    t_obs_wo = t_obs_wo,
    t_obs_nb = t_obs_nb,
    # P-value summaries for each metric
    wr = pv_wr,
    wo = pv_wo,
    nb = pv_nb,
    # Raw permutation distributions (for diagnostics / Q-Q plots)
...
# Win Ratio
res$perm$wr$k             # number of permutations as extreme as observed
res$perm$wr$p_empirical   # (k+1)/(B+1); conservative, always > 0
res$perm$wr$z_score       # z-score for reporting
res$perm$wr$p_asymptotic  # 2*Phi(-|z|); extrapolated, assumes normal null

# Win Odds
res$perm$wo$p_empirical
res$perm$wo$z_score
res$perm$wo$p_asymptotic

# Net Benefit
res$perm$nb$p_empirical
res$perm$nb$z_score
res$perm$nb$p_asymptotic

# --- Diagnostic: check normality of permutation null ---
# If roughly linear, asymptotic p is credible
...
save(res,file = here::here("inst","CVC_Eperke_WinRatio","iter4_state_end.RData"))
